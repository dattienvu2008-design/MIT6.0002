...
class Position(object):
    ...
    def __str__(self):  
        return "Position: " + str(
            math.floor(self.x)) + ", " + str(math.floor(self.y))
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for i in range(width):
            for k in range(height):
                self.tile_dirt_amount[(i,k)] = dirt_amount
                #May change (i,k) -> Position(i,k)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tile_dirt_amount[(x, y)] <= capacity:
            self.tile_dirt_amount[(x, y)] = 0
        else:
            self.tile_dirt_amount[(x, y)] -= capacity
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.tile_dirt_amount[m,n] == 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        for i in range(self.width):
            for k in range(self.height):
                if self.tile_dirt_amount[(i,k)] == 0:
                    num_cleaned_tiles += 1
    ...
    def is_position_in_room(self, pos):
        ...
        """
        return pos.get_x() < (self.width) and pos.get_y() < (
            self.height) and pos.get_x() >= 0 and (pos.get_y()
                                                   >= 0)
    ...
    def get_dirt_amount(self, m, n):
        ...
        Returns: an integer
        """
        return self.tile_dirt_amount.get((m,n))
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        """
        self.room = room
        self.angle = self.get_robot_direction()
        self.speed = speed
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        # May add code to check if pos in the room in the future?
    ...
    def get_random_position(self):
        ...
        while y == self.height:
            y = random.uniform(0, self.height)
        while self.is_position_furnished(Position(x, y)) is True:
            x = random.uniform(0, self.width)
            while x == self.width:
                x = random.uniform(0, self.width)
            y = random.uniform(0, self.height)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        speed = self.speed
        new_pos = self.get_robot_position().get_new_position(self.angle, speed)
        if self.room.is_position_valid(new_pos) is True:
            self.set_robot_position(new_pos)
            self.room.clean_tile_at_position(new_pos, self.capacity)
        if self.room.is_position_valid(new_pos) is False:
            self.set_robot_direction(self.get_robot_direction())
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if faulty is False:
            speed = self.speed
            new_pos = self.get_robot_position().get_new_position(
                                                    self.angle, speed)
            if self.room.is_position_valid(new_pos) is True:
                self.set_robot_position(new_pos)
                self.room.clean_tile_at_position(new_pos, self.capacity)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        while room.get_num_cleaned_tiles()/num_tiles < min_coverage:
            ...
            time_step += 1
    avg_time_step = time_step/num_trials
...
def show_plot_room_shape(title, x_label, y_label):
    ...
    for width in [10, 20, 25, 50]:
        ...
        times1.append(run_simulation(2, 1.0, 1, width, int(height), 3, 0.8, 200, StandardRobot))
