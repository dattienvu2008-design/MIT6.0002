...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        # TODO
        return f'{self.src.get_name()}->{self.dest.get_name()} ({self.total_distance}, {self.outdoor_distance})'
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        if self.has_node(edge.get_source()) == False or self.has_node(edge.get_destination()) == False:
            raise ValueError
        if self.edges.get(edge.get_source()) != None:
            self.edges[edge.get_source()].append(edge)
        elif self.edges.get(edge.get_source()) == None:
